...
```{r}
pg_crime_data <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/pg_crime_totals_2024.csv")
...
```{r}
#### From a reporting standpoint, I believe that violent crimes such as assaults, accidents, homicides, and sex offenses are the most important to cover. Public safety is a crucial concern, especially when these incidents occur in local communities.Following these violent crimes, the frequency of robberies is an issue. A news story could focus on the reasons behind these robberies and explore how it can be prevented. Additionally, it would be valuable to examine how these incidents may relate to some of the violent crimes.
...
#The highest category is accidents, with an average of 481 incidents per month. The lowest category is residential robbery, with an average of about four incidents per month. The data offers limited insight for accidents, because the category is broad. However, the robbery data is more gives me more detail showing that thefts are more likely to occur at businesses than in homes.
