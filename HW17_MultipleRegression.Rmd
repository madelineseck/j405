...
##When examining the correlation matrix, some independent variables, such as having a college degree or higher, income, and age, are correlated with the dependent variable, voter turnout, and also show a linear relationship with it. Some variables, like population and unemployment rate, do not appear to be correlated with voter turnout.
```{r}
# Summary statistics
summary(md_turnout)
...
##The model predicts turnout well. Predictors such as income, education level, and median age show strong positive linear relationships with voter turnout. Variables like unemployment and population appear to be the least significant predictors. Overall, the model is useful because it provides a clear visual representation of which factors strongly correlate with turnout.
...
# Model 3
model3 <- lm(turnout ~ income * pct_bachelors_higher + median_age + pct_white, data = md_turnout)
summary(model3)
...
##I would report on how some communities are overrepresented or underrepresented in election results due to differences in voter turnout.